...
if(is_array($arResult["MORE_PHOTO"]) && count($arResult["MORE_PHOTO"]) > 0) {
	...
	//§5 ДЕДУП ГАЛЕРЕИ: убрать дубли главного фото и повторы в доп-фото.
	// 1) точное совпадение ID файла; 2) одинаковый контент (md5) при совпадении размера.
	// md5 считается только если файл доступен (is_file) — безопасно и при отсутствии оригиналов.
	$vmdDetailId   = isset($arResult["DETAIL_PICTURE"]["ID"]) ? (int)$arResult["DETAIL_PICTURE"]["ID"] : 0;
	$vmdDetailSize = isset($arResult["DETAIL_PICTURE"]["FILE_SIZE"]) ? (int)$arResult["DETAIL_PICTURE"]["FILE_SIZE"] : 0;
	$vmdDetailHash = null;
	if($vmdDetailSize > 0 && !empty($arResult["DETAIL_PICTURE"]["SRC"])) {
		$vmdDetailPath = $_SERVER["DOCUMENT_ROOT"].$arResult["DETAIL_PICTURE"]["SRC"];
		if(is_file($vmdDetailPath)) { $vmdDetailHash = md5_file($vmdDetailPath); }
	}
	$vmdSeen = array();
	foreach($arResult["MORE_PHOTO"] as $key => $arFile) {
		$fid = isset($arFile["ID"]) ? (int)$arFile["ID"] : 0;
		$dup = false;
		if($fid && $fid === $vmdDetailId)          $dup = true;
		elseif($fid && isset($vmdSeen[$fid]))      $dup = true;
		if(!$dup && $vmdDetailHash !== null && !empty($arFile["SRC"])
			&& (int)($arFile["FILE_SIZE"] ?? 0) === $vmdDetailSize) {
			$p = $_SERVER["DOCUMENT_ROOT"].$arFile["SRC"];
			if(is_file($p) && md5_file($p) === $vmdDetailHash) $dup = true;
		}
		if($dup) { unset($arResult["MORE_PHOTO"][$key]); continue; }
		if($fid) $vmdSeen[$fid] = true;
	}
...
$arResult["PROPERTIES"]["ARTNUMBER"] = $arResult["PROPERTIES"]["CML2_ARTICLE"];
