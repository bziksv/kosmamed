...
//CATALOG_DETAIL//?>
<?
if($arResult["MIN_PRICE"]["BASE_PRICE"]==888888888 || $arResult["TOTAL_OFFERS"]["MIN_PRICE"]["BASE_PRICE"]==888888888) {
	$arResult["CAN_BUY"]=false;
	$arResult["TOTAL_OFFERS"]["QUANTITY"]=0;
	$arResult["TOTAL_OFFERS"]["MIN_PRICE"]["RATIO_PRICE"]=0;
	$arResult["MIN_PRICE"]["RATIO_PRICE"]=0;
	$sticker=false;
}
?>
	...
	<div class="catalog-detail">
		<div class="column first<?=($arResult["COLLECTION"]["THIS"]) ? " colletion" : ""?>">
			<div class="catalog-detail-pictures">
				<?if(!empty($arResult["PROPERTIES"]["VIDEO"]) || $arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0] || count($arResult["MORE_PHOTO"]) > 0) {?>
					...
					<div class="more_photo">
						<ul>
							...
							if($arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"]) {
								foreach($arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"] as $PHOTO) {

										$arFileTmpb = CFile::ResizeImageGet(
											$PHOTO,
											array("width" => 560, "height" => 747),
											BX_RESIZE_IMAGE_PROPORTIONAL,
											true
										);
										$arPHOTOb = array(
											"SRC" => $arFileTmpb["src"],
											"WIDTH" => $arFileTmpb["width"],
											"HEIGHT" => $arFileTmpb["height"],
										);


										$arFileTmp = CFile::ResizeImageGet(
											$PHOTO,
											array("width" => 146, "height" => 195),
											BX_RESIZE_IMAGE_PROPORTIONAL,
											true
										);
										$arPHOTO = array(
									...
									<li class="<?=$active; $active='';?>" style="<?=($arParams['DISPLAY_MORE_PHOTO_WIDTH'] ? 'width:'.$arParams['DISPLAY_MORE_PHOTO_WIDTH'].'px;' : '').($arParams['DISPLAY_MORE_PHOTO_HEIGHT'] ? 'height:'.$arParams['DISPLAY_MORE_PHOTO_HEIGHT'].'px;' : '');?>">
										<a  class="catalog-detail-images  111" >
											<img src="<?=$arPHOTO['SRC']?>" width="<?=$arPHOTO['WIDTH']?>" height="<?=$arPHOTO['HEIGHT']?>" alt="<?=$arResult['NAME']?>" title="<?=$arResult['NAME']?>" />
										</a>
									</li>
								<?}
							}?>
							...
							<?if(!empty($arResult["PROPERTIES"]["VIDEO"]["VALUE"])) {?>
								<li class="<?=$active; $active='';?>" class="catalog-detail-video" style="<?=($arParams['DISPLAY_MORE_PHOTO_WIDTH'] ? 'width:'.$arParams['DISPLAY_MORE_PHOTO_WIDTH'].'px;' : '').($arParams['DISPLAY_MORE_PHOTO_HEIGHT'] ? 'height:'.$arParams['DISPLAY_MORE_PHOTO_HEIGHT'].'px;' : '');?>">
									<a rel="lightbox" class="catalog-detail-images  222" href="#video">
						...
						foreach($arResult["OFFERS"] as $key => $arOffer) {
							...
							<div id="detail_picture_<?=$arItemIDs['ID'].'_'.$arOffer['ID']?>" class="detail_picture<?=($key == $arResult['OFFERS_SELECTED'] ? '' : ' hidden');?>">
								<?if($isOfferDetailImg || $isDetailImg) {?>
									<?/*<a <?=($key == $arResult['OFFERS_SELECTED'] ? 'rel="lightbox" ' : '');?>class="catalog-detail-images fancybox 888" id="catalog-detail-images-<?=$arItemIDs['ID'].'-'.$arOffer['ID']?>" href="<?=($isOfferDetailImg ? $arOffer['DETAIL_PICTURE']['SRC'] : $arResult['DETAIL_PICTURE']['SRC']);?>">*/?>
									<div <?=($key == $arResult['OFFERS_SELECTED'] ? 'rel="lightbox" ' : '');?>class="catalog-detail-images  888" id="catalog-detail-images-<?=$arItemIDs['ID'].'-'.$arOffer['ID']?>" >
								<?} else {?>
									<div class="catalog-detail-images">
								<?}
								if($isOfferDetailImg) {?>
									<img src="<?=$arOffer['DETAIL_IMG']['SRC']?>" width="<?=$arOffer['DETAIL_IMG']['WIDTH']?>" height="<?=$arOffer['DETAIL_IMG']['HEIGHT']?>" alt="<?=$offerName?>" title="<?=$offerName?>" fetchpriority="high" decoding="async" />
								<?} elseif($isDetailImg) {?>
									<img src="<?=$arResult['DETAIL_IMG']['SRC']?>" width="<?=$arResult['DETAIL_IMG']['WIDTH']?>" height="<?=$arResult['DETAIL_IMG']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" fetchpriority="high" decoding="async" />
								<?} else {?>
									<img class="no_photo_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
								<?}?>
								...
								<?}
								unset($arVendor);?>
								<?=($isOfferDetailImg || $isDetailImg ? "</div>" : "</div>");?>
								<?//=($isOfferDetailImg || $isDetailImg ? "<div class='icon_zoom'><i class='icon-scale-plus' data-entity='slider-magnifier'></i></div></a>" : "</div>");?>
							</div>
						<?}
						unset($offerName, $isOfferDetailImg);
					//DETAIL_PICTURE//
					} else {?>
						<?if($arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0]){

							$arFileTmpp = CFile::ResizeImageGet(
								$arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0],
								array("width" => 560, "height" => 747),
								BX_RESIZE_IMAGE_PROPORTIONAL,
								true
							);
							...
							?>
							<meta content="<?=($isDetailImg ? $arPHOTOp['SRC'] : SITE_TEMPLATE_PATH.'/images/no-photo.svg');?>" itemprop="image" />
							<div class="detail_picture <?if(!empty($arResult["PROPERTIES"]["VIDEO"]) || $arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0] || count($arResult["MORE_PHOTO"]) > 0) echo "detail_picture_pa";?>">
								
								<?foreach($arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"] as $cell => $PHOTO) {
									$arFileTmpp = CFile::ResizeImageGet(
										$PHOTO,
										array("width" => 560, "height" => 747),
										BX_RESIZE_IMAGE_PROPORTIONAL,
									...
									?>
									<?if($arPHOTOp['SRC']) {?>
										<?/*<a <?if($activeDN):?>style="display: none;"<?else: $activeDN=true;?><?endif;?>  rel="lightbox" class="catalog-detail-images fancybox 333" href="<?=$arPHOTOp['SRC']?>">*/?>
										<div <?if($activeDN):?>style="display: none;"<?else: $activeDN=false;?><?endif;?>  rel="lightbox" class="catalog-detail-images 333">
											<img src="<?=$arPHOTOp['SRC']?>" width="<?=$arFileTmpp['WIDTH']?>" height="<?=$arFileTmpp['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
									<?} /*else {?>
										<div class="catalog-detail-images no_images 111">
											<img class="no_photo_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
									<?}*/?>
									<div class="time_buy_sticker">
										<?=$timeBuy?>
									...
									unset($arVendor);?>
									<?=($arPHOTOp['SRC'] ? "</div>" : "");?>
									<?//=($arPHOTOp['SRC'] ? "<div class='icon_zoom'><i class='icon-scale-plus' data-entity='slider-magnifier'></i></div></a>" : "");?>
									<?//=($isDetailImg ? "<div class='icon_zoom'><i class='icon-scale-plus' data-entity='slider-magnifier'></i></div></a>" : "</div>");?>
								...
								if($arOffer['DETAIL_PICTURE']['SRC'] || $arResult['DETAIL_PICTURE']['SRC']){
									if($isDetailImg) {?>
										<?/*<a <?if($activeDN):?>style="display: none;"<?else: $activeDN=true;?><?endif;?> rel="lightbox" class="catalog-detail-images fancybox 777" href="<?=$arResult['DETAIL_PICTURE']['SRC']?>">*/?>
										<div <?if($activeDN):?>style="display: none;"<?else: $activeDN=false;?><?endif;?> rel="lightbox" class="catalog-detail-images 777 99">
											<img src="<?=$arResult['DETAIL_IMG']['SRC']?>" width="<?=$arResult['DETAIL_IMG']['WIDTH']?>" height="<?=$arResult['DETAIL_IMG']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" fetchpriority="high" decoding="async" />
									<?} else {?>
										<div class="catalog-detail-images no_images 222">
											<img class="no_photo_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
									<?}?>
									<div class="time_buy_sticker">
										<?=$timeBuy?>
								...
								if(count($arResult["MORE_PHOTO"]) > 0) {
									foreach($arResult["MORE_PHOTO"] as $PHOTO) {
										?>
										<?if($isDetailImg) {?>
											<?/*<a <?if($activeDN):?>style="display: none;"<?else: $activeDN=true;?><?endif;?> rel="lightbox" class="catalog-detail-images fancybox 333" href="<?=$PHOTO['SRC']?>">*/?>
											<div <?if($activeDN):?>style="display: none;"<?else: $activeDN=false;?><?endif;?> rel="lightbox" class="catalog-detail-images 333">
												<img src="<?=$PHOTO['SRC']?>" width="<?=$arResult['DETAIL_IMG']['WIDTH']?>" height="<?=$arResult['DETAIL_IMG']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
										<?} else {?>
											<div class="catalog-detail-images no_images 3333">
												<img class="no_photo_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
										<?}?>
										<div class="time_buy_sticker">
											<?=$timeBuy?>
						...
						<?} else {?>
							<meta content="<?=($isDetailImg ? $arResult['DETAIL_PICTURE']['SRC'] : SITE_TEMPLATE_PATH.'/images/no-photo.svg');?>" itemprop="image" />
							<div class="detail_picture <?if(!empty($arResult["PROPERTIES"]["VIDEO"]) || $arResult["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0] || count($arResult["MORE_PHOTO"]) > 0) echo "detail_picture_pa";?>">
								...
								if($arOffer['DETAIL_PICTURE']['SRC'] || $arResult['DETAIL_PICTURE']['SRC']){
									if($isDetailImg) {?>
										<?/*<a <?if($activeDN):?>style="display: none;"<?else: $activeDN=true;?><?endif;?> rel="lightbox" class="catalog-detail-images fancybox 777" href="<?=$arResult['DETAIL_PICTURE']['SRC']?>">*/?>
										<div <?if($activeDN):?>style="display: none;"<?else: $activeDN=false;?><?endif;?> rel="lightbox" class="catalog-detail-images  777 000" >
											<img src="<?=$arResult['DETAIL_IMG']['SRC']?>" width="<?=$arResult['DETAIL_IMG']['WIDTH']?>" height="<?=$arResult['DETAIL_IMG']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" fetchpriority="high" decoding="async" />
									<?} else {?>
										<div class="catalog-detail-images no_images 444">
											<img class="no_photo_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
									<?}?>
									<div class="time_buy_sticker">
										<?=$timeBuy?>
								...
								<?}else{?>
									<?if(count($arResult["MORE_PHOTO"]) == 0) {?>
									<?if($isDetailImg) {?>
										<?/*<a rel="lightbox" class="catalog-detail-images fancybox 777" href="<?=$arResult['DETAIL_PICTURE']['SRC']?>">*/?>
										<div rel="lightbox" class="catalog-detail-images 777 88" >
											<img src="<?=$arResult['DETAIL_IMG']['SRC']?>" width="<?=$arResult['DETAIL_IMG']['WIDTH']?>" height="<?=$arResult['DETAIL_IMG']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" fetchpriority="high" decoding="async" />
									<?} else {?>
									...
									foreach($arResult["MORE_PHOTO"] as $PHOTO) {
										...
										<?//if($isDetailImg) {?>
											...
											<div <?if($activeDN):?>style="display: none;"<?else: $activeDN=false;?><?endif;?> rel="lightbox" class="catalog-detail-images 333" >
												<img src="<?=$PHOTO['SRC']?>" width="<?=$arResult['DETAIL_IMG']['WIDTH']?>" height="<?=$arResult['DETAIL_IMG']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
										<?/*} else {?>
											<div class="catalog-detail-images no_images 666">
												<img class="no_photo_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
										<?}*/?>
										<div class="time_buy_sticker">
											<?=$timeBuy?>
										...
										<?}
										unset($arVendor);?>
										<?/*<div class='icon_zoom'><i class='icon-scale-plus' data-entity='slider-magnifier'></i></div>*/?>
										</div>
									...
									<div class="gift-image-cont">
										<div class="gift-image">
											<div class="gift-image-col">
												...
												<?} else {?>
													<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="70" height="70" alt="<?=$arGift['NAME']?>" title="<?=$arGift['NAME']?>" />
												<?}?>
											</div>
										</div>
									</div>
					...
					<div class="price_buy_detail" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
						...
						<div class="catalog-detail-price" id="<?=$arItemIDs['PRICE'];?>">
							...
							//DETAIL_PRICE//
								...
								<?//OTHER_PRICE//?>
								<?if($arResult["PRICE_MATRIX_SHOW"]["COLS"] && count($arResult["PRICE_MATRIX_SHOW"]["COLS"]) > 1) {?>
									<div class="catalog-detail-price-ranges other-price">
								...
								<div class="article" style="float: none;"><?=GetMessage("CATALOG_ELEMENT_ARTNUMBER")?><?=!empty($arResult["PROPERTIES"]["ARTNUMBER"]["VALUE"]) ? $arResult["PROPERTIES"]["ARTNUMBER"]["VALUE"] : "-";?></div>
								<div class="article code_product" style="float: none;">: <?=$arResult["ID"]?></div>
...
if($_SERVER["HTTP_COOKIE"]){
	$arCClID = explode('BITRIX_SM_GEOLOCATION_LOCATION_ID=',$_SERVER["HTTP_COOKIE"]);
	if($arCClID[1]){
		$arrCClID = explode(';',$arCClID[1]);
	}
}

if ($USER->IsAuthorized() || $arrCClID):?>
						<div id="<?=$arItemIDs['DELIVERY']?>" class="geolocation_delivery_from">
							<?if($arParams["AJAX_MODE"] == "Y") {?>
										...
										if($arSetting["OFFERS_VIEW"]["VALUE"] != "LIST"):
											...
											//if($arOffer["CAN_BUY"] && $arOffer["ITEM_PRICES"][$arOffer["ITEM_PRICE_SELECTED"]]["RATIO_PRICE"] > 0):?>
												<div id="geolocation-delivery-from" class="geolocation-delivery-from">
													<?$APPLICATION->IncludeComponent("altop:geolocation.delivery", "delivery",
														array(
...
<?endif;?>
					...
					*/?>
					<?//DETAIL_PREVIEW_TEXT//
					if($arResult["PROPERTIES"]["DESC_DETAIL"]["~VALUE"]){?>
						<div class="catalog-detail-preview-text" itemprop="description">
							<?=$arResult["PROPERTIES"]["DESC_DETAIL"]["~VALUE"]["TEXT"]?>
						</div>
						<?
					}
							...
							<div class="catalog-item-info">
								...
								<div class="catalog-item-image-cont">
									<div class="catalog-item-image">
										...
										<?} else {?>
											<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="72" height="72" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
										<?}?>
										<div class="sticker">
											<?=$sticker?>
										</div>
	...
	<?}*/
	//DETAIL_KIT_ITEMS//
	if($arResult["KIT_ITEMS"] && count($arResult["KIT_ITEMS"]) > 0) {?>
		<div class="kit-items">
						...
						<div class="catalog-item-info">
							...
							<div class="item-image-cont">
								<div class="item-image">
									...
									<?} else {?>
										<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
											<img class="item_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$arItem['NAME']?>" title="<?=$arItem['NAME']?>" />
										</a>
									<?}?>
								</div>
							</div>
	...
	<div class="tabs-wrap tabs-catalog-detail">
		...
		<div class="tabs__box" <?=(!$arResult["COLLECTION"]["THIS"]) ? " style='display:block;'" : ""?>>
			<div class="tabs__box-content">
				...
				<?if($arResult["PROPERTIES"]["DESC"]["~VALUE"]) echo $arResult["PROPERTIES"]["DESC"]["~VALUE"]["TEXT"];?>
			</div>
		</div>
		<?//PROPERTIES_TAB//
					...
					<div class="catalog-detail-properties">
						...
						if(!empty($arResult["DISPLAY_PROPERTIES"])) {
							foreach($arResult["DISPLAY_PROPERTIES"] as $k => $v) {
								if($v["NAME"]=="Характеристики"):
									foreach($v["DISPLAY_VALUE"] as $hk => $hv) :?>
										<div class="catalog-detail-property">
											<div class="name"><?=$hv?></div>
											<div class="dots"></div>
											<div class="val"><?=$v["DESCRIPTION"][$hk]?></div>
										</div>
									<?endforeach;?>
								<?else:?>
									<div class="catalog-detail-property">
...
<?if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"])) {
	$arJSParams = array(
		...
		"PRODUCT" => array(
			"ID" => $arResult["ID"],
			"NAME" => $arResult["~NAME"],
			"PICT" => is_array($arResult["PREVIEW_IMG"]) ? $arResult["PREVIEW_IMG"] : array("SRC" => SITE_TEMPLATE_PATH."/images/no-photo.svg", "WIDTH" => 150, "HEIGHT" => 150)
		),
