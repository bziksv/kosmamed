...
<div id="catalog-section-list" class="catalog-section-list">
	<?foreach($arResult["SECTIONS"] as $arSection) {
		...
		<div class="catalog-section  <?if(count($arSection["CHILDREN"])>9):?>o_h_s<?endif;?>">
			...
			if($bHasChildren) {?>
							...
							<a href="<?=$arChild['SECTION_PAGE_URL']?>" title="<?=$arChild['NAME']?>">
								<span class="child">
									<span class="graph">
										...
										<?} else {?>
											<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="50" height="50" alt="<?=$arChild['NAME']?>" title="<?=$arChild['NAME']?>" />
										<?}?>
									</span>
				...
				<?if(count($arSection["CHILDREN"])>9):?>
					<div class="catalog_more catalog_more_section">
						<button type="button" class="btn_buy apuo">Показать ещё</button>
					</div>
				<?endif;?>
			<?}?>
		</div>	
