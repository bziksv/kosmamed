...
<div id="<?=$strMainID?>_info" class="item_info" style="display: none">	
		...
		if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"]) && $arSetting["OFFERS_VIEW"] != "LIST") {
			foreach($arResult["OFFERS"] as $key_off => $arOffer) {
				...
				<div id="img_<?=$arItemIDs['ID']?>_<?=$arOffer['ID']?>" class="img<?=($key_off == $arResult['OFFERS_SELECTED'] ? '' : ' hidden');?>">
					...
					<?} else {?>
						<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
					<?}?>
				</div>
			<?}
	...
	<div class="item_block<?=(isset($arResult['OFFERS']) && !empty($arResult['OFFERS']) && $arSetting['OFFERS_VIEW'] == 'LIST' ? ' offers-list' : '');?>">
		...
		if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"]) && $arSetting["OFFERS_VIEW"] != "LIST") {
			$arSkuProps = array();?>
			<table class="offer_block 2222" id="<?=$arItemIDs['PROP_DIV'];?>">			
		...
		if(isset($arResult["SELECT_PROPS"]) && !empty($arResult["SELECT_PROPS"])) {
			$arSelProps = array();?>
			<table class="offer_block 1111" id="<?=$arItemIDs['SELECT_PROP_DIV'];?>">
											...
											<?} else {
												...
												<div class="add2basket_block">
													...
													<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
														...
														<?}?>
														<button type="button" class="btn_buy s_add_b" name="add2basket"><i class="fa fa-shopping-cart"></i></button>
													</form>
										...
										<div class="add2basket_block">
											<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
												...
												<?if(!empty($arResult["SELECT_PROPS"])) {?>
													<input type="hidden" name="SELECT_PROPS" id="select_props_<?=$arItemIDs['ID'].'_'.$arOffer['ID']?>" value="" />
												<?}?>
												<button type="button" class="btn_buy s_add_b" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=($arSetting["NAME_BUTTON_TO_CART"] ? $arSetting["NAME_BUTTON_TO_CART"] : Loc::getMessage("CATALOG_ELEMENT_ADD_TO_CART"))?></span></button>
											</form>
										</div>
									<?}
								} elseif(!$arOffer["CAN_BUY"]) {
							...
							<div class="add2basket_block">
								<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
									...
									<?if(!empty($arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"])) {
										$props = array();
										$props[] = array(
											"NAME" => $arResult["PROPERTIES"]["CML2_ARTICLE"]["NAME"],
											"CODE" => $arResult["PROPERTIES"]["CML2_ARTICLE"]["CODE"],
											"VALUE" => $arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]
										);												
									...
									<button type="button" id="<?=$arItemIDs['BTN_BUY']?>" class="btn_buy s_add_b" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=($arSetting["NAME_BUTTON_TO_CART"] ? $arSetting["NAME_BUTTON_TO_CART"] : Loc::getMessage("CATALOG_ELEMENT_ADD_TO_CART"))?></span></button>
								</form>
							</div>
						</div>
					<?}?>
...
if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"])) {	
	$arJSParams = array(
		...
		"PRODUCT" => array(
			"ID" => $arResult["ID"],
			"NAME" => $arResult["NAME"],
			"PICT" => is_array($arResult["PREVIEW_PICTURE"]) ? $arResult["PREVIEW_PICTURE"] : array("SRC" => SITE_TEMPLATE_PATH."/images/no-photo.svg", "WIDTH" => 150, "HEIGHT" => 150),
...
<script type="text/javascript">
	...
	console.log(1111);
	$( ".s_add_b" ).prop('disabled', false).trigger( "click" ).removeClass("s_add_b");
	$( "#altop_search form .fa-times" ).trigger( "click" );

	$(document).on("click",".tvr_search .btn_buy", function () {
		$( ".s_add_b" ).prop('disabled', false).trigger( "click" ).removeClass("s_add_b");
		console.log(2222);
	});
