...
<div class="catalog-item-info infographics_info">
	<?//QUICK_VIEW?>
	<?if($inQuickView){?>
	...
	<div class="catalog-item-image-cont infographic_list 666">
...
if($arElement["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0]){
		$arFileTmpp = CFile::ResizeImageGet(
			$arElement["PROPERTIES"]["INFOGRAPHICS"]["VALUE"][0],
			array("width" => 7500, "height" => 10000),
		...
		?>
			<a class="infographics_list" href="<?=$arElement['DETAIL_PAGE_URL']?>">
				<?if(($arPHOTOp["SRC"])) {?>
					<img class="item_img" src="<?=$arPHOTOp['SRC']?>" width="<?=$arElement['PREVIEW_PICTURE']['WIDTH']?>" height="<?=$arElement['PREVIEW_PICTURE']['HEIGHT']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
				<?} else {?>
					<img class="item_img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="150" height="150" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
				<?}?>
				<div class="sticker-cont catalog-item-image">
					<?=$timeBuy?>
					<span class="sticker">
						<?=$sticker?>
					</span>
		...
		//ITEM_PRICE//
			...
			<?} else {?>
				<span class="catalog-item-price">
					...
					<?}?>
					<?if(is_array($arElement["PRICE_MATRIX"]["MATRIX"]) && is_array($arElement["ITEM_QUANTITY_RANGES"]) && count($arElement["PRICE_MATRIX"]["MATRIX"]) > 1 && count($arElement["ITEM_QUANTITY_RANGES"]) <= 1) {?>
						<span class="catalog-item-price-ranges-wrap">
							<a id="<?=$itemIds['PRICE_RANGES_BTN']?>" class="catalog-item-price-ranges" href="javascript:void(0);"><i class="fa fa-question-circle-o"></i></a>
						</span>
					<?}?>
