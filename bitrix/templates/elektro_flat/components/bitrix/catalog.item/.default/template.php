...
if(isset($arResult['ITEM'])) {
	...
	$arElement["PROPERTIES"]["ARTNUMBER"] = $arElement["PROPERTIES"]["CML2_ARTICLE"];
	...
	//PRICE_MATRIX//
	if($arElement["PRICE_MATRIX_SHOW"]["COLS"] && count($arElement["PRICE_MATRIX_SHOW"]["COLS"]) > 1 && empty($arElement["CURRENT_DISCOUNT"]["ACTIVE_TO"])) {
		$class = " item-pm";
	}
	if($haveOffers) {
		if($arElement["TOTAL_OFFERS"]["PRICE_MATRIX_SHOW"]["COLS"] && count($arElement["TOTAL_OFFERS"]["PRICE_MATRIX_SHOW"]["COLS"]) > 1 && empty($arElement["CURRENT_DISCOUNT"]["ACTIVE_TO"])) {
			$class = " item-pm";
		}
	}
