...
if(!$arElement["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]){
	...
	{	
		if($ob['VALUE']) $arElement["PROPERTIES"]["CML2_ARTICLE"] = $ob;
	}
}
?>
	...
	if($ob['VALUE']) $arElement["PROPERTIES"]["MORE_PHOTO"]["VALUE"][] = $ob['VALUE'];
			...
			<div class="item-image">
				...
				<a href="<?=$arElement['DETAIL_PAGE_URL']?>">
					<?if(($arPHOTOp[0]["SRC"])) {
						?>
						<div class="magic_slide_ss 33 44">
						<?
	...
	<div class="items_block_desc">
		<?//ITEM_TITLE//?>
	        <div class="item-all-title product-item-preview-full" >
	            ...
	            <div class="product-item-preview-full-text 22222" >
<?
...
?>
		...
		if($inArticle || $inRating) {?>
			<div class="article_rating">
				...
				if($inArticle) {?>
					<div class="article">
	                    <? if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_ARTICLE_ATTR"])): ?>
		                ...
		                <? else: ?>
		                    <span class="show_article"><?=!empty($arElement["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) ? $arElement["PROPERTIES"]["CML2_ARTICLE"]["VALUE"] : "-";?></span>
		                <? endif; ?>
					</div>
...
{    
    if($ob['VALUE']) $arElement["PROPERTIES"]["DESC_DETAIL"]["~VALUE"] = $ob['~VALUE'];
		...
		<?//OTHER_PRICE//
		if($haveOffers) {
			if($arElement["TOTAL_OFFERS"]["PRICE_MATRIX_SHOW"]["COLS"] && count($arElement["TOTAL_OFFERS"]["PRICE_MATRIX_SHOW"]["COLS"]) > 1) {?>
						 ...
						 <?if($arParams['SHOW_MAX_QUANTITY'] !== 'N') { ?>
	                        <div class="avl">
								...
								<? if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_AVAILABLE_ATTR"])): ?>
									<span class="show_in_stock 666"></span>
								<? else: ?>
									<span>
										<?=$arResult["SECTION"]["UF_AVAILABLE_ATTR"]?>
									</span>
								<? endif; ?>
			...
			<?//ITEM_AVAILABILITY_BUY//
				...
				<div class="add2basket_block">
					<?if($arElement["CAN_BUY"]) {
	                    ...
	                    <?if(isWrapAttr($arResult["COUNTER"], $arResult["SECTION"]["UF_ORDER_ATTR"])):?>
				<a id="<?=$itemIds['POPUP_BTN']?>" class="btn_buy apuo" href="javascript:void(0)" rel="nofollow" data-action="under_order"><i class="fa fa-clock-o"></i><span><?=Loc::getMessage("CT_BCS_ELEMENT_UNDER_ORDER")?></span></a>
	                    <? else: ?>
				<a id="<?=$itemIds['POPUP_BTN']?>" class="btn_buy apuo" href="javascript:void(0)" rel="nofollow" data-action="under_order"><i class="fa fa-clock-o"></i><span data-order-show="<?=$arResult["SECTION"]["UF_ORDER_ATTR"]?>" class="show_order"></span></a>
	                    <? endif; ?>
					<?}?>
				</div>
			<?}?>
