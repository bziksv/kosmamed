...
// LCP preload: первый слайд выводится как CSS background — подсказываем браузеру приоритет загрузки
$vmdFirstSlide = $arResult['ITEMS'][array_key_first($arResult['ITEMS'])];
if(!empty($vmdFirstSlide['PICTURE_PREVIEW']['SRC'])) {
	echo '<link rel="preload" as="image" href="'.htmlspecialcharsbx($vmdFirstSlide['PICTURE_PREVIEW']['SRC']).'" fetchpriority="high">'."\n";
}
?>
