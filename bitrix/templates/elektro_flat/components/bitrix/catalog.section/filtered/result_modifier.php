...
// VMD §1.1: кешируемая «случайность» вместо ORDER BY RAND().
// Пул грузится кешируемой сортировкой (SORT), перемешивание и выборка N — в PHP.
// Включается только когда вызов передаёт VMD_RANDOM=Y (см. PERFORMANCE-CHECKLIST §1.1).
if (isset($arParams["VMD_RANDOM"]) && $arParams["VMD_RANDOM"] === "Y"
	&& !empty($arResult["ITEMS"]) && is_array($arResult["ITEMS"])) {
	shuffle($arResult["ITEMS"]);
	$vmdCount = isset($arParams["VMD_RANDOM_COUNT"]) ? (int)$arParams["VMD_RANDOM_COUNT"] : 0;
	if ($vmdCount > 0 && count($arResult["ITEMS"]) > $vmdCount) {
		$arResult["ITEMS"] = array_slice($arResult["ITEMS"], 0, $vmdCount);
	}
}
...
foreach($arResult["ITEMS"] as $key => $arElement) {
	...
	$arResult["ITEMS"][$key]["PROPERTIES"]["ARTNUMBER"] = $arElement["PROPERTIES"]["CML2_ARTICLE"];
