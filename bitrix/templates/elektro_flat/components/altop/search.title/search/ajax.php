...
$secN=0;
	...
	$arFilter = Array('IBLOCK_ID'=>24, 'GLOBAL_ACTIVE'=>'Y', 'NAME'=>$arResult["query"].'%');
	$db_list = CIBlockSection::GetList(Array($by=>$order), $arFilter, true, ['NAME','CODE','ELEMENT_CNT','PICTURE']);
	while($ar_result = $db_list->GetNext())
	{
		if($ar_result['ELEMENT_CNT']>0 && $secN<5){
			$secN++;
		?>
		<div class="tvr_search 123 tvr_search_s">						
			<a class="image" href="/catalog/<?=$ar_result['CODE']?>/">
				<?if($ar_result['PICTURE']):
					$src = CFile::GetPath($ar_result["PICTURE"]);?>
				<img src="<?=$src;?>" width="62" height="41" alt="<?=$ar_result['NAME']?>" title="<?=$ar_result['NAME']?>">
				<?else:?>
					<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="62" height="62" alt="<?=$ar_result['NAME']?>" title="<?=$ar_result['NAME']?>" />
				<?endif;?>
			</a>
			<div class="cat_title">
				<a href="/catalog/<?=$ar_result['CODE']?>/"><?=$ar_result['NAME']?></a>
			</div>						
		</div>
		<?
		}
...
$fTitle = explode($arResult["query"],$arElement["NAME"]);

$fTitleA = explode($arResult["query"],$arElement["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]);

$fTitleID = explode($arResult["query"],$arElement["ITEM_ID"]);
...
<?*/
if( (count($fTitle)<2 && $arElement["NAME"]) && (count($fTitleA)<2) && (count($fTitleID)<2) ) continue;
				...
				if($category_id === "all") {
					...
					<div class="tvr_search 123">						
						...
						<a class="image" href="<?=$arElement['URL']?>">
							...
							<?} else {?>
								<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.svg" width="62" height="62" alt="<?=$arElement['NAME']?>" title="<?=$arElement['NAME']?>" />
							<?}?>
						</a>
						...
						if($arElement["CHECK_QUANTITY"] && $arElement["CATALOG_QUANTITY"]>0){
						?>
						<div class="available">
														                            <div class="avl">
						                            <i class="fa fa-check-circle"></i>
						                            <span>
						                                В наличии  <?=$arElement["CATALOG_QUANTITY"]?>                            </span>
						                        </div>
						                     							</div>
						<?
						}else{
						...
						<div class="not_avl">
							...
							<span class="show_out_of_stock"></span>
											</div>
						<?
						}
						...
						if($arParams["SHOW_ADD_TO_CART"] == "Y") {
							...
							if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) {?>
								<div class="buy_more">
									<div class="add2basket_block 111">
										<form action="<?=$curPage?>" class="add2basket_form">
											...
											<button type="button" id="<?=$arItemIDs['PROPS_BTN']?>" class="btn_buy" name="add2basket"><i class="fa fa-shopping-cart"></i><span>В корзину</span></button>
										</form>
									</div>
								</div>
							<?//ITEM_BUY//
								...
								if($arElement["CAN_BUY"] && !$arElement["COLLECTION"]) {?>
									<div class="buy_more">
										<div class="add2basket_block 222">
											<?if($arElement["MIN_PRICE"]["DISCOUNT_VALUE"] <= 0) {
												//ITEM_ASK_PRICE//?>
												<form action="javascript:void(0)" class="apuo_form">										
													...
													<button type="button" id="<?=$arItemIDs['POPUP_BTN']?>" class="btn_buy apuo 222" ><i class="fa fa-comment-o"></i><span class="short"><?=GetMessage("CATALOG_ELEMENT_ASK_PRICE_SHORT")?></span></button>
												</form>
											<?} else {
								...
								<?/*
									...
									*/?>				
									...
									<input type="hidden" name="action_<?=$arElement['ITEM_ID']?>" value="props" />
													...
													<?//}?>															
													<button type="button" id="<?=(isset($arElement['SELECT_PROPS']) && !empty($arElement['SELECT_PROPS']) ? $arItemIDs['PROPS_BTN'] : $arItemIDs['BTN_BUY']);?>" class="btn_buy" name="add2basket"><i class="fa fa-shopping-cart"></i><span>В корзину</span></button>
												</form>
											<?}?>
										</div>
									</div>
									...
									<div class="buy_more">
										...
										<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
												<?} else {?>
													<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_search_form">
												<?}?>
											<input type="hidden" name="action_<?=$arElement['ITEM_ID']?>" value="under_order" />

											<input type="hidden" name="area_id_<?=$arElement['ITEM_ID']?>" value="<?=$arItemIDs['PROPS_BTN']?>" />
									...
									*/?>				
									<input type="hidden" name="ID" value="<?=$arElement['ITEM_ID']?>"/>
													<?//if(!isset($arElement["SELECT_PROPS"]) || empty($arElement["SELECT_PROPS"])) {?>
														...
														<input type="hidden" name="PROPS" value="<?=$props?>" />
													<?//}?>															
													<button action="under_order" type="button" id="<?=(isset($arElement['SELECT_PROPS']) && !empty($arElement['SELECT_PROPS']) ? $arItemIDs['PROPS_BTN'] : $arItemIDs['BTN_BUY']);?>" class="btn_buy apuo" name="add2basket"><i class="fa fa-shopping-cart"></i><span>Под заказ</span></button>
												</form>
