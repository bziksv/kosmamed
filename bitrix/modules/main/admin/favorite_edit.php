...
function UserInfo($USER_ID)
{
	...
	if($user_arr = $user->Fetch())
		return '[<a title="'.GetMessage("MAIN_USER_PROFILE").'" href="user_edit.php?ID='.$user_arr["ID"].'&amp;lang='.LANGUAGE_ID.'">'.$user_arr["ID"].'</a>] ('.htmlspecialcharsbx($user_arr["LOGIN"]).') '.htmlspecialcharsbx($user_arr["NAME"]).' '.htmlspecialcharsbx($user_arr["LAST_NAME"]);
}
...
if($_SERVER['REQUEST_METHOD']=="POST" && (!empty($_POST['save']) || !empty($_POST['apply'])) && check_bitrix_sessid())
{
	...
	if($res)
	{
		if($apply <> "")
		{
			...
			LocalRedirect(($_REQUEST["addurl"]<>""? $_REQUEST["addurl"]:"favorite_list.php?lang=".LANGUAGE_ID));
	}
	else
	{
...
?>
	...
	<tr>
		...
		<td><?=$str_TIMESTAMP_X?> / <?= UserInfo($str_MODIFIED_BY)?></td>
	</tr>
	<?php endif; ?>
	<?php if ($str_DATE_CREATE <> '') : ?>
	<tr>
		<td><?= GetMessage("MAIN_CREATED")?></td>
		<td><?=$str_DATE_CREATE?> / <?= UserInfo($str_CREATED_BY)?></td>
	</tr>
	<?php endif; ?>
	...
	<tr >
		...
		<td><?= CLanguage::SelectBox("LANGUAGE_ID", $str_LANGUAGE_ID)?></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><textarea name="COMMENTS" class="textarea" rows="5" cols="40"><?= $str_COMMENTS?></textarea></td>
	</tr>
<?php
if($isAdmin):
?>
...
while($ar = $a->fetch()):
?>
	<option value="<?= htmlspecialcharsbx($ar["REFERENCE_ID"])?>"<?php if($ar["REFERENCE_ID"] == $str_MODULE_ID) echo " selected"?>><?= htmlspecialcharsbx($ar["REFERENCE"])?></option>
<?php endwhile?>
</select>
<script>
...
$tabControl->Buttons(array(
	"disabled"=>false,
	"back_url"=>($_REQUEST["addurl"]<>""? $_REQUEST["addurl"]:"favorite_list.php?lang=".LANGUAGE_ID),
));
$tabControl->End();
?>
</form>
