...
?>
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%"><input type="text" name="NAME" size="40" maxlength="100" value="<?= CTask::GetLangTitle($str_NAME, $str_MODULE_ID);?>"></td>
	</tr>
	<tr class="adm-detail-required-field">
		...
		<select name="MODULE_ID" id="__module_id_select">
			<option value="main" <?= ($str_MODULE_ID == 'main') ? 'selected' : '';?>><?=GetMessage('KERNEL')?></option>
		<?php
		...
		?>
			...
			<option value="<?=htmlspecialcharsbx($MID)?>"<?= ($str_MODULE_ID == $MID? ' selected' : '');?>><?=htmlspecialcharsbx($m->MODULE_NAME);?></option>
		<?php endforeach;?>
		</select>
		</td>
	</tr>
	<tr>
		...
		<td><?= ($str_SYS == 'Y') ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO")?></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><textarea name="DESCRIPTION" cols="30" rows="5"><?= CTask::GetLangDescription($str_NAME, $str_DESCRIPTION, $str_MODULE_ID);?></textarea>
		</td>
	</tr>
	...
	<tr>
		...
		<table border="0" cellpadding="5" cellspacing="0" align="center">
			...
			foreach($arOperations as $arOperation)
			{
				...
				<?= (($arOperation["MODULE_ID"] != $str_MODULE_ID) || ($arOperation["BINDING"] != $str_BINDING)) ? 'style="display: none"' : ''?>>
					<td align="right" style="padding: 0 10px 0 10px">
						<input type="checkbox" name="OPERATION_ID[]" id="OPERATION_ID_<?=$ind ?>" value="<?=$arOperation["ID"]?>" <?= (in_array($arOperation["ID"], $arTaskOperations)) ? " checked" : ''?>>
						<script>
						arOperations['<?=$ind?>'] = {
