...
if ($_SERVER['REQUEST_METHOD'] === "POST" && !empty($_POST['Update']) && $isAdmin && check_bitrix_sessid())
{
	...
	$CONDITION_OLD = $_POST['CONDITION_OLD'] ?? '';
	...
	if (empty($aMsg))
	{
		if (empty($_POST['apply']))
			LocalRedirect("/bitrix/admin/urlrewrite_list.php?lang=".LANGUAGE_ID."&filter_site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false));
	}
	else
	{
...
if ($bVarsFromForm)
{
	$str_CONDITION_OLD = htmlspecialcharsbx($_POST['CONDITION_OLD']);
	$str_CONDITION = htmlspecialcharsbx($CONDITION);
	$str_ID = htmlspecialcharsbx($_POST['ID']);
	$str_FILE_PATH = htmlspecialcharsbx($_POST['FILE_PATH']);
	$str_RULE = htmlspecialcharsbx($_POST['RULE']);
}

$aMenu = array(
	array(
		"TEXT" => GetMessage("MURL_2_LIST"),
		"LINK" => "/bitrix/admin/urlrewrite_list.php?lang=".LANGUAGE_ID."&filter_site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false),
		"ICON"	=> "btn_list",
...
if (!empty($CONDITION))
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/bitrix/admin/urlrewrite_edit.php?lang=".LANGUAGE_ID."&site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false),
		"ICON"	=> "btn_new",
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessage("MURL_ACT_DEL_CONF")."')) window.location='/bitrix/admin/urlrewrite_list.php?ID=".urlencode(urlencode($CONDITION))."&filter_site_id=".urlencode(urlencode($site_id))."&action=delete&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
		"WARNING" => "Y",
		"ICON"	=> "btn_delete"
	);
}
...
$tabControl->Buttons(
	array(
		"disabled" => !$isAdmin,
		"back_url" => "/bitrix/admin/urlrewrite_list.php?lang=".LANGUAGE_ID."&filter_site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false)
	)
);
?>
<?php
