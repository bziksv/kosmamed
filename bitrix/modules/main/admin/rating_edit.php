...
if($_SERVER['REQUEST_METHOD']=="POST" && (!empty($_POST['save']) || !empty($_POST['apply'])) && check_bitrix_sessid())
{
	...
	if($res)
	{
		if (!empty($_POST["apply"]))
		{
			...
			LocalRedirect(($_REQUEST["addurl"]<>""? $_REQUEST["addurl"]:"rating_list.php?lang=".LANGUAGE_ID));
	}
	else
	{
								...
								<td class="rating-table-component-table-td rating-table-component-table-td-1">
									...
									for ($i=0; $i<$FIELD_COUNT; $i++)
									{
										if (isset($arConfig['FIELDS'][$i]['TYPE']) && $arConfig['FIELDS'][$i]['TYPE'] == 'SELECT_ARRAY_WITH_INPUT')
										{
											...
											?>
											<tr valign="top">
												<td class="rating-table-component-table-td-table-td rating-table-component-table-td-table-td-1 field-name" style="vertical-align:middle"><label><?= $arConfig['FIELDS'][$i]['NAME'] ?? GetMessage('RATING_FIELDS_DEF_NAME') ?></label></td>
