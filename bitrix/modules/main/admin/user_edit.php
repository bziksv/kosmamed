...
if($strError <> '' || !$res)
{
	...
	$GROUP_ID_NUMBER = intval($_POST["GROUP_ID_NUMBER"] ?? 0);
	$str_GROUP_ID = array();
	for ($i = 0; $i <= $GROUP_ID_NUMBER; $i++)
	{
		if (isset($_POST["GROUP_ID_ACT_".$i]) && $_POST["GROUP_ID_ACT_".$i] == "Y")
		{
			$str_GROUP_ID[intval($_POST["GROUP_ID_".$i])]["DATE_ACTIVE_FROM"] = $_POST["GROUP_ID_FROM_".$i];
			$str_GROUP_ID[intval($_POST["GROUP_ID_".$i])]["DATE_ACTIVE_TO"] = $_POST["GROUP_ID_TO_".$i];
		}
	}
}
...
?>
	<tr>
		...
		<td>
		<?php if($canSelfEdit):?>
			<input type="checkbox" name="BLOCKED" value="Y"<?php if($str_BLOCKED == "Y") echo " checked"?>>
		<?php else:?>
			<input type="checkbox" <?php if($str_BLOCKED == "Y") echo " checked"?> disabled>
...
?>
	<tr id="bx_pass_row" style="display:<?=($str_EXTERNAL_AUTH_ID <> ''? 'none':'')?>;"<?php if($ID<=0 || $COPY_ID>0):?> class="adm-detail-required-field"<?php endif?>>
		...
		<td><input type="password" name="NEW_PASSWORD" size="30" maxlength="255" value="<?= htmlspecialcharsbx($NEW_PASSWORD ?? '') ?>" autocomplete="new-password" style="vertical-align:middle;">
<?php if($bSecure):?>
...
<?php endif?>
	...
	<tr id="bx_pass_confirm_row" style="display:<?=($str_EXTERNAL_AUTH_ID <> ''? 'none':'')?>;"<?php if($ID<=0 || $COPY_ID>0):?> class="adm-detail-required-field"<?php endif?>>
		...
		<td><input type="password" name="NEW_PASSWORD_CONFIRM" size="30" maxlength="255" value="<?= htmlspecialcharsbx($NEW_PASSWORD_CONFIRM ?? '') ?>" autocomplete="new-password"></td>
	</tr>
<?php
		...
		<td colspan="2" align="center">
			...
			while ($arGroups = $dbGroups->Fetch())
			{
				...
				<tr>
					...
					<td class="align-left">
						<label for="GROUP_ID_ACT_ID_<?= $ind ?>"><?=htmlspecialcharsbx($arGroups["NAME"])?> [<a href="/bitrix/admin/group_edit.php?ID=<?=$arGroups["ID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("MAIN_VIEW_GROUP")?>"><?= intval($arGroups["ID"])?></a>]</label>
					</td>
					<td>
...
?>
	<tr>
		<td><?= $tabControl->GetCustomLabelHTML()?></td>
		<td><?= SelectBoxFromArray("PERSONAL_COUNTRY", GetCountryArray(), $str_PERSONAL_COUNTRY, GetMessage("USER_DONT_KNOW"));?></td>
	</tr>
<?php
...
?>
	<tr>
		<td><?= $tabControl->GetCustomLabelHTML()?></td>
		<td><?= SelectBoxFromArray("WORK_COUNTRY", GetCountryArray(), $str_WORK_COUNTRY, GetMessage("USER_DONT_KNOW"));?></td>
	</tr>
<?php
...
?>
	<tr>
		<td align="center" colspan="2"><textarea name="ADMIN_NOTES" cols="50" rows="10" style="width:100%;"><?= $str_ADMIN_NOTES?></textarea></td>
	</tr>
<?php
