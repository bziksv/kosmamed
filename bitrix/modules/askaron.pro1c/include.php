			...
			if (  COption::GetOptionString("askaron.pro1c", "fast_write") == "Y" )
			{
				if( $_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import') )
				{
					...
					if ( $arFields["IBLOCK_ID"] > 0 && $arFields["IBLOCK_ID"] == $arCatalogInfo["ADDITIONAL"]["PRODUCT_IBLOCK_ID"]  )
					{
						...
						// === НАЧАЛО ИЗМЕНЕНИЙ ===
						// Очищаем описания, чтобы они не импортировались
						unset($arFields["DETAIL_TEXT"]);
						unset($arFields["DETAIL_TEXT_TYPE"]);
						unset($arFields["PREVIEW_TEXT"]);
						unset($arFields["PREVIEW_TEXT_TYPE"]);
						
						// Если нужно полностью отказаться и от свойств, раскомментируйте следующую строку,
						// чтобы они не восстанавливались в OnAfterWriteElement.
						// $arFields["ASKARON_PRO1C_SKIP_PROPERTIES"] = true;
						// === КОНЕЦ ИЗМЕНЕНИЙ ===
					}
				}			
			}
	...
	private static function OnAfterWriteElement(&$arFields, $action )
	{
		if (self::IsExchange())
		{
			...
			if ( $bWriteOk && isset( $arFields["ASKARON_PRO1C_TMP_PROPERTY_VALUES"] ) )
			{
				...
				// === НАЧАЛО ИЗМЕНЕНИЙ ===
				// Проверяем, не хотим ли мы полностью пропустить свойства
				if ( isset( $arFields["ASKARON_PRO1C_SKIP_PROPERTIES"] ) && $arFields["ASKARON_PRO1C_SKIP_PROPERTIES"] === true )
				{
					// Просто ничего не делаем, свойства не восстанавливаем
					self::log("Properties skipped for element ID=".$ELEMENT_ID);
				}
				else
				{
				// === КОНЕЦ ИЗМЕНЕНИЙ ===
