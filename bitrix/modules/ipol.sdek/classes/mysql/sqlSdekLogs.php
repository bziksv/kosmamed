<?php
	class sqlSdekLogs{
		...
		public static function getAccountsList($labeled=false,$all=false,$keys=false){
			global $DB;
			$active = ($all) ? '' : "WHERE ACTIVE='Y'";
			$res = $DB->Query("SELECT ID, ACCOUNT, SECURE, LABEL FROM ".self::$tableName." ".$active, false, "File: ".__FILE__."<br>Line: ".__LINE__);
			...
			while($acc=$res->Fetch())
				if ($labeled) {
                    ...
                    if ($keys) {
                        $arAccs[$acc['ID']]['SECURE'] = $acc['SECURE'];
                    }
                } else {
