...
(function (exports, main_core, im_v2_application_core, im_v2_component_elements_button, im_v2_component_message_base, im_v2_component_message_default, im_v2_component_message_elements, im_v2_lib_utils) {
	'use strict';
	...
	function openLinkInSlider(link) {
		...
		return BX.SidePanel.Instance.open('sign:stub:sign-link', {
			...
			contentCallback(slider) {
				return BX.Runtime.loadExtension('sign.v2.b2e.sign-link').then(exports$1 => {
					const memberIdFromLinkToSigning = /\/sign\/link\/member\/(\d+)\//i.exec(link);
					return new exports$1.SignLink({
	...
	const SignMessage = {
		...
		methods: {
			replacePhrase(phrase) {
				let text = phrase ?? '';
				const userLink = im_v2_lib_utils.Utils.user.getProfileLink(this.user?.id);
				const initiatorLink = im_v2_lib_utils.Utils.user.getProfileLink(this.initiator?.id);
				...
				const phrases = {
					'#DOCUMENT_NAME#': `<span class="${DOCUMENT_CLASS}">${main_core.Text.encode(this.document?.name)}</span>`,
					'#USER_LINK#': `<a href="${userLink}" class="${LINK_CLASS}">${main_core.Text.encode(this.user?.name)}</a>`,
					'#INITIATOR_LINK#': `<a href="${initiatorLink}" class="${LINK_CLASS}">${main_core.Text.encode(this.initiator?.name)}</a>`,
...
})(this.BX.Messenger.v2.Component.Message = this.BX.Messenger.v2.Component.Message || {}, BX, BX.Messenger.v2.Application, BX.Messenger.v2.Component.Elements, BX.Messenger.v2.Component.Message, BX.Messenger.v2.Component.Message, BX.Messenger.v2.Component.Message, BX.Messenger.v2.Lib);
