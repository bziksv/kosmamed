...
(function (exports,im_v2_lib_desktopApi,main_date,im_v2_lib_dateFormatter,im_v2_const,main_core) {
	...
	const TextUtil = {
	  ...
	  getMentionBbCode(dialogId, name) {
	    if (main_core.Type.isString(dialogId) && Utils.dialog.isChatDialogId(dialogId)) {
	...
	const DialogUtil = {
	  ...
	  isChatDialogId(dialogId) {
	    return dialogId.startsWith(im_v2_const.DialogIdChatPrefix);
	  },
	  getChatIdFromDialogId(dialogId) {
	    if (!this.isChatDialogId(dialogId)) {
	      throw new Error('Provided dialogId is not correct');
	    }
	    return Number(dialogId.slice(im_v2_const.DialogIdChatPrefix.length));
	  },
	  buildChatDialogId(chatId) {
	    return `${im_v2_const.DialogIdChatPrefix}${chatId}`;
	  },
