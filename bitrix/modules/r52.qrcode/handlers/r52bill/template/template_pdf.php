...
if($params['QR_STR']) {
    ...
    $params['QR_CODE'] = $qr->saveQrLogo($res, 'bill');
//    $params['QR_CODE'] = '/upload/qrcode/bill.png';
}
...
if ($params['BILL_HEADER_SHOW'] == 'Y')
{
	...
	if($params['QR_CODE']) {
        ...
        list($qrHeight, $qrWidth) = $pdf->GetImageSize($params['QR_CODE']);

        $imgDpi = intval($params['BILL_LOGO_DPI']) ?: 96;
        $imgZoom = 96 / $imgDpi;

        $qrHeight = $qrHeight * $imgZoom + 5;
        $qrWidth  = $qrWidth * $imgZoom + 5;

        $pdf->Image($params['QR_CODE'], ($pageWidth - 100 - $margin['right'] + 5), $pdf->GetY(), 100, 100);
	...
	$pdf->Ln();
	$pdf->SetY(max($y0 + $qrHeight, $pdf->GetY()));
