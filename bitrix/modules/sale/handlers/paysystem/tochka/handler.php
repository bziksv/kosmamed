...
class TochkaHandler extends PaySystem\ServiceHandler //implements PaySystem\IRefund
{
	...
	public function initiatePay(Payment $payment, Request $request = null)
	{
        ...
        $form_url = $busValues["TOCHKA_FORM_URL"];
        $no_vat = $busValues["TOCHKA_NO_VAT"];
        $this->setPaymentFormUrl($form_url);
        $login = $busValues["TOCHKA_LOGIN"];
        $secret = $busValues["TOCHKA_SECRET"];
        $orderid = ($this->getPaymentFormType() == "create") ? $payment->getOrderId() : $payment->getOrderId() . "|tochka_payment_system";
        ...
        $service_name = "tochka_payment_system";
		...
		$params = array(
            ...
            "login" => $this->getOrderParams("login"),
            "cart" => $this->fiscal_cart
        );
        if($busValues["TOCHKA_PSTYPE"] != ""){
            $params["pstype"] = $busValues["TOCHKA_PSTYPE"];
            }
	...
	public function refund(Payment $payment, $refundableSum): PaySystem\ServiceResult
	{
        ...
        $busValues = $this->getParamsBusValue($payment);
        $tochka_url = $busValues["TOCHKA_FORM_URL"];
        $tochka_login = $busValues["TOCHKA_LK_LOGIN"];
        $tochka_password = $busValues["TOCHKA_LK_PASSWORD"];
        if($pos=strrpos($tochka_url, "/create"))
        {
            $tochka_url = substr($tochka_url, 0, $pos);
        }
        else if($pos=strrpos($tochka_url, "/order"))
        {
            $tochka_url = substr($tochka_url, 0, $pos);
        }
        $url = $this->getUrl($payment, 'test');
        $tochka_id = substr($payment->getField('PS_STATUS_MESSAGE'), 12); 
        ...
        $refund=$this->refundTochka($tochka_login, $tochka_password, $tochka_url, $params);
        $message = "Tochka refund - order_id ".$orderid.", sum ".$totalsum.", status ".$refund->result." cart ".$this->fiscal_cart;
    ...
    private function refundTochka($user, $password, $server_tochka, array $params) 
    {
    ...
    curl_setopt($curl,CURLOPT_URL,$server_tochka.$uri);
    ...
    {   
        if($request->get('action') == "tochka_cancel"){
	...
	public function processRequest(Payment $payment, Request $request)
	{
        ...
        else
        {
            ...
            $status_id_after_payment = $busValues["TOCHKA_STATUS_AFTER_PAYMENT"];
    ...
    {
        if (strpos($request->get('orderid'), 'tochka_payment_system') !== false) {
    ...
    public function setOrderParams($order_total = 0, $clientid="", $orderid="", $client_email="", 
                                    $client_phone="", $service_name="", $form_url="", $login="", $secret_key="")
    {
