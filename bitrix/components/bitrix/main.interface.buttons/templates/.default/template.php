...
<div
	...
	<div class="main-buttons-box" role="list">
			...
			?><div
				...
				if (!$arItem["HTML"]):
					if (!empty($arItem["URL"])):
						...
						?><button type="button" tabindex="0" <?=$attrs?> class="main-buttons-item-link<?=$arParams["CLASS_ITEM_LINK"] ? " ".$arParams["CLASS_ITEM_LINK"] : ""?>"><?
					endif
					...
					if ($arItem["SUB_LINK"]):
						?><a tabindex="-1" aria-hidden="true" class="main-buttons-item-sublink<?=" ".($arItem["SUB_LINK"]["CLASS"] ?? '')?>" href="<?=htmlspecialcharsbx($arItem["SUB_LINK"]["URL"])?>"></a><?
					endif;
				else:
					echo $arItem["HTML"];
				endif;
							...
							<? foreach ($arItem["CHILD_ITEMS"] as $childKey => $arChildItem) :
								...
								if (!$arChildItem["HTML"]):
									...
									if ($arChildItem["SUB_LINK"]):
										?><a tabindex="-1" aria-hidden="true" class="main-buttons-item-sublink<?=" ".($arChildItem["SUB_LINK"]["CLASS"] ?? '')?>" href="<?=htmlspecialcharsbx($arChildItem["SUB_LINK"]["URL"])?>"></a><?
									endif;
								else:
		...
		<? if (!$arResult["MORE_BUTTON"]["HTML"]):
			?><button type="button" tabindex="0" aria-haspopup="true" aria-expanded="false" class="main-buttons-item-link<?=$arParams["CLASS_ITEM_LINK"] ? " ".$arParams["CLASS_ITEM_LINK"] : ""?>"><?
				?><span class="main-buttons-item-icon<?=$arParams["CLASS_ITEM_ICON"] ? " ".$arParams["CLASS_ITEM_ICON"] : ""?>"></span><?
				?><span class="main-buttons-item-text<?=$arParams["CLASS_ITEM_TEXT"] ? " ".$arParams["CLASS_ITEM_TEXT"] : ""?>"><?
					?><span class="main-buttons-item-text-title"><?
